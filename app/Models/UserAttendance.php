...
class UserAttendance extends Model
{
    ...
    public function getIsTardyAttribute(){
        ...
        if(!$this->user->shift_id) return 'Shift not set';
        ...
        $time_in = $this->timeToSeconds($this->time_in);
        //get the time in of the shift in seconds
        $shift_time_in = $this->timeToSeconds($shift->start_time);
        //calculate the difference between the time in of the user and the time in of the shift
        $diff = $time_in-$shift_time_in  ;
        //if $diff is greater than 0 and less than 8 hours, return $diff in hh:mm:ss format, else return 'No'
